...
import { Map } from "../Map"
import { useUsers, usersToListItems, usersToMapItems } from "~/controllers/users"
import { useChargers, chargersToListItems, chargersToMapItems } from "~/controllers/chargers"
import { useStations, stationsToListItems, stationsToMapItems } from "~/controllers/stations"
      ...
      <div className="flex w-full h-full pt-2 content-center justify-center flex-row">
        <div className="h-full w-3/12 px-1.5">
          <DashboardBox title={"Controls"}>
            ...
            <SwitchControl title="Users" valueId="users" onChange={setShowUsers}/>
            <SwitchControl title="Chargers" valueId="charger" onChange={setShowChargers} />
            <SwitchControl title="Stations" valueId="stations" onChange={setShowStations}/>
          </DashboardBox>
        </div>
  ...
  return (
    <ListEntry>
      <p className="text-white text-xl w-20 h-full text-bottom">
        ...
        <Switch aria-label={props.valueId} onChange={(e) => {
